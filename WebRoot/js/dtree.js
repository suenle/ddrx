...
function Node(id, pid, name, url, title, target, icon, iconOpen, open) {
	...
	this._io = open || true;	// open对应_io，表示节点是否已经打开

	this._is = false;			// 节点是否被选中，默认值false

	this._ls = false;			// 是否是最后一个节点，默认值false

	this._hc = false;			// 是否有子节点，默认值false
...
function dTree(objName) {
	...
	this.config = {
		...
		folderLinks		: true,		//文件夹可被链接

		useSelection	: true,		//节点可被选择高亮

		useCookies		: true,		//创建带结构连接线的树

		useLines		: true,		//创建带结构连接线的树

		useIcons		: true,		//创建带有图表的树

		useStatusText	: false,	//用节点名替代显示在状态栏的节点url

		closeSameLevel	: true,		//点击文件夹的时候，自动打开本文件夹的子目录,同时关闭同层其他打开的文件夹

		inOrder			: true		//加速父节点树的显示
